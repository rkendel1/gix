...
pub(crate) fn rewrite_url_aliases_with_error_kind(
    ...
    urls.iter()
        .map(|url| rewrite_url(config, Some(url), direction, error_kind))
        .collect()
}
...
pub(crate) fn rewrite_url_aliases_non_destructive_with_error_kind(
    ...
    let aliases = urls
        .iter()
        .map(|url| match rewrite_url(config, Some(url), direction, error_kind) {
            Ok(alias) => alias,
            Err(err) => {
...
pub(crate) fn rewrite_url_aliases_with_fallback_non_destructive(
    ...
    let aliases = urls
        .iter()
        .map(|url| match rewrite_url(config, Some(url), direction, direction) {
            Ok(Some(alias)) => Some(alias),
            Ok(None) => match rewrite_url(config, Some(url), fallback, direction) {
                Ok(alias) => alias,
                Err(err) => {
                    first_error.get_or_insert(err);
                    None
