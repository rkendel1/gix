...
impl File {
    ...
    fn raw_value_filter_inner(
        ...
        for section_id in section_ids.rev() {
            ...
            if let Some(v) = section
                .body
                .value_implicit_in(&self.event_backing, value_name)
                .flatten()
            {
                return Ok(v);
            }
    ...
    fn raw_value_mut_filter_inner(
        ...
        let mut section_ids = self
            ...
            .rev();
        let key = section::ValueName::try_from(value_name).expect("value names from AsKey are validated");
            ...
            for (i, event) in section.as_ref().iter().enumerate() {
                match event {
                    Event::SectionValueName(event_key)
                        if event_key.eq_ignore_ascii_case_in(&self.event_backing, &key) =>
                    {
                        found_key = true;
                        index = i;
                        size = 1;
    ...
    fn raw_values_filter_inner(
        ...
        for section_id in section_ids {
            ...
            values.extend(section.body.values_in(&self.event_backing, value_name));
        }
    ...
    fn raw_values_mut_filter_inner(
        ...
        if entries.is_empty() {
            ...
            Ok(MultiValueMut {
                ...
                backing: &mut self.event_backing,
                key,
