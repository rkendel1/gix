mod lookup_ref_delta_objects {
    ...
    fn entry(header: Header, data: &'static [u8]) -> input::Entry {
        ...
        let mut entry =
            input::Entry::from_data_obj(&obj, 0, gix_features::zlib::Compression::BEST_SPEED).expect("valid object");
        entry.header = header;
    ...
    fn only_ref_deltas_are_handled() -> crate::Result {
        ...
        let actual = LookupRefDeltaObjectsIter::new(
            ...
            gix_features::zlib::Compression::BEST_SPEED,
        )
    ...
    fn ref_deltas_have_their_base_injected_if_not_done_before_and_all_future_entries_are_offset() {
        ...
        let iter = LookupRefDeltaObjectsIter::new(input_entries, &db, gix_features::zlib::Compression::BEST_SPEED);
        assert_eq!(
            iter.size_hint(),
    ...
    fn lookup_errors_trigger_a_fuse_and_stop_iteration() {
        ...
        let mut result = LookupRefDeltaObjectsIter::new(
            into_results_iter(input),
            &db,
            gix_features::zlib::Compression::BEST_SPEED,
        )
